...
class NetSalesReportController extends Controller
{
    public function show(Request $request) {
        ...
        if($request->ajax()) {
         $returnSub = DB::connection("mysql2")
            ...
            return view("Reports.net_sales_report.custom_sales_tax_report_ajax", compact("net_sales_reports"));
        }

        return view("Reports.net_sales_report.custom_sales_tax_report");
    }
    ...
    public function NetSalesExecutiveReport(Request $request) {
        ...
        if($request->ajax()) {
            $net_sales_reports = DB::connection("mysql2")->table("subitem")
                                        ->select(
                                            "subitem.product_name AS product_name",
                                            "brands.name AS brand_name",
                                            "brands.id",
            ...
            return view("Reports.net_sales_report.custom_sales_tax_report_ajax_e", compact("net_sales_reports"));
        }

        return view("Reports.net_sales_report.custom_sales_tax_report_e");
    }
}
