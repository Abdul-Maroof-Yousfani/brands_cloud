...
// }
    ...
    public function getUserStockReportApi(Request $request)
    {
        ...
        $stockData = DB::connection('mysql2')->table('ba_stock')
            ...
            ->select(
                'sub_item_id',
                DB::raw('SUM(CASE WHEN voucher_type IN (51,1,9) AND transfer_status != 1 THEN qty ELSE 0 END) AS total_in'),
                DB::raw('SUM(CASE WHEN voucher_type IN (50) THEN qty ELSE 0 END) AS total_out')
            )
            ->groupBy('sub_item_id')
            ->get()
            ->keyBy('sub_item_id');
