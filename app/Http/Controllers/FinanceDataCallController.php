...
class FinanceDataCallController extends Controller
{
     ...
     */
     ...
     public function showadvancepayment(){
        return view('Finance.AjaxPages.showadvancepayment');
    }
    ...
    public function getJvsDateAndAccontWise(Request $request)
    {
        ...
        if($amount == ''){$Clause2 = '';}
        else{$Clause2 = 'AND b.amount = '.$amount;}
        ...
        $NewJvs=$NewJvs->join('new_jv_data','new_jv_data.master_id','=','new_jvs.id')->select('new_jvs.*')->where('new_jvs.status',1)->whereBetween('new_jvs.jv_date',array($FromDate,$ToDate));
        if($VoucherStatus !="")
        {
            $NewJvs=$NewJvs->where('new_jvs.jv_status',$VoucherStatus);
        }
        if($amount != ""){
            $NewJvs=$NewJvs->where('new_jv_data.amount',$amount);
        }
        if($jv_no != ""){
            $NewJvs=$NewJvs->whereRaw('LOWER(new_jvs.jv_no) LIKE ?',['%'.strtolower($jv_no).'%']);
        }
        // if($username != ""){
        //     $NewJvs=$NewJvs->orWhere('new_jvs.username', $username);
        // }
        if ($username != "") {
            $NewJvs = $NewJvs->where('new_jvs.username', $username);
        }
        $NewJvs=$NewJvs->groupBy('new_jv_data.master_id')->get();
    ...
    public function getBpvsDateAndAccontWise(Request $request)
    {
        ...
        if($ref_no == ''){$Clause2 = '';}
        else{$Clause2 = 'AND a.bill_no = '.$ref_no;}
        ...
        $pvs=$pvs->join('new_pv_data','new_pv_data.master_id','=','new_pv.id')->select('new_pv.*')->where('new_pv.status',1)->where('new_pv.payment_type',1)->where('new_pv.type',1)->whereBetween('new_pv.pv_date',array($FromDate,$ToDate));
        if($VoucherStatus !="")
        {
            $pvs=$pvs->where('new_pv.pv_status',$VoucherStatus);
        }
        if($amount != ""){
            $pvs=$pvs->where('new_pv_data.amount',$amount);
        }
        if($ref_no != ""){
            $pvs=$pvs->where('new_pv.bill_no',$ref_no);
        }
    ...
    public function getCpvsDateAndAccontWise(Request $request)
    {
        ...
        $pvs=$pvs->join('new_pv_data','new_pv_data.master_id','=','new_pv.id')->select('new_pv.*')->where('new_pv.status',1)->where('new_pv.payment_type',2)->where('new_pv.type',1)->whereBetween('new_pv.pv_date',array($FromDate,$ToDate));
        if($VoucherStatus !="")
        {
            $pvs=$pvs->where('new_pv.pv_status',$VoucherStatus);
        }
    ...
    public function getCrvsDateAndAccontWise(Request $request)
    {
        ...
        $NewRvs=$NewRvs->join('new_rv_data','new_rv_data.master_id','=','new_rvs.id')->select('new_rvs.*')->where('new_rvs.status',1)->where('new_rvs.rv_status',$VoucherStatus)->where('new_rvs.rv_type',2)->whereBetween('new_rvs.rv_date',array($FromDate,$ToDate));
        if($VoucherStatus !="")
        {
            $NewRvs=$NewRvs->where('new_rvs.rv_status',$VoucherStatus);
        }
        if($amount != ""){
            $NewRvs=$NewRvs->where('new_rv_data.amount',$amount);
        }
        if($ref_no != ""){
            $NewRvs=$NewRvs->where('new_rvs.ref_bill_no',$ref_no);
        }
            $NewRvs=$NewRvs->groupBy('new_rv_data.master_id')->get();
    ...
    public function getBrvsDateAndAccontWise(Request $request)
    {
        ...
        if($AccountId !=""):
           ...
           $NewRvs= DB::Connection('mysql2')->select('select a.* from new_rvs a
            ...
            $NewRvs=$NewRvs->join('new_rv_data','new_rv_data.master_id','=','new_rvs.id')->select('new_rvs.*')->where('new_rvs.status',1)->where('new_rvs.rv_type',1)->where('new_rvs.sales','!=',1)->whereBetween('new_rvs.rv_date',array($FromDate,$ToDate));
            if($VoucherStatus !="")
            {
                $NewRvs=$NewRvs->where('new_rvs.rv_status',$VoucherStatus);
            }
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                <table class="table table-bordered sf-table-th sf-table-list profit_Loss_Statement"style="pading:10px 8px !important;" id="exportIncomeStatement1" style="background:#FFF !important;">
                            ...
                            <?php
                            CommonHelper::companyDatabaseConnection($CompanyId);
                            $accounts = DB::select("SELECT * FROM accounts WHERE level1=5  order by level1,level2,level3,level4,level5,level6,level7");
                            $counter = 1;
                            CommonHelper::reconnectMasterDatabase();
                            foreach($accounts as $row):
                            $head=strlen($row->code);
