...
@section('content')
    ...
    <div id="content" class="well_N">
        <div class="row no-print">
            <div class="col-lg-12">
                <input type="button" class="btn btn-sm btn-success"
                    onclick="tablesToExcel(array1, 'Ledger_Report', 'Ledger_Report.xls')" value="Export to Excel" />
                <button type="button" class="btn btn-sm btn-primary" onclick="window.print()">Print PDF / Print</button>
                <span class="pull-right text-muted" style="font-size: 12px;">Report Generated On:
                    {{ date('d-M-Y H:i:s') }}</span>
            </div>
        </div>
        <div class="lineHeight">&nbsp;</div>
        <div class="no-print">
            <p class="text-danger" style="font-size: 14px; font-weight: bold;">
                <!-- <i class="fa fa-info-circle"></i> All Ledgers Must have PDF export + report print date time stamp -->
            </p>
        </div>
        <?php
    $all_debit = 0;
    ...
    $total_purchase_invoice = 0;
        ...
        @foreach($data1 as $row1)
            ...
            <table class="table table-bordered sf-table-th sf-table-list"
                id="export_table_to_excel_<?php    echo $LedgerCount; ?>">
                <?php
            ...
            CommonHelper::reconnectMasterDatabase();
                ...
                <thead>
                    <tr>
                        <td colspan="8" style="background-color:#ffffff; border: none !important;">
                            <h2 class="ledger-title">{{$row1->code . ' ' . $row1->name}}</h2>
                        </td>
                    </tr>
                </thead>
                <thead>
            ...
            foreach ($quarter as $trow):
                ...
                if ($trow->voucher_type == 13):
                    $detail = 'sales/viewDeliveryNoteDetail';
                    $page_typ = 'Delivery Note';
                    $total_debit_note += $trow->amount;
                ...
                if ($trow->voucher_type == 15):
                    $detail = 'sales/viewCreditNoteDetail';
                    $page_typ = 'Sales Return after Sales Tax Invoice';
                    $total_debit_note += $trow->amount;

                    $PageTitle = 'Sales Return after Sales Tax Invoice';
                    $type = 'Sales Return after Sales Tax Invoice';
                endif;
                ...
                if ($trow->voucher_type == 2):
                    ...
                    <tr title="<?php        echo $trow->voucher_type ?>" class="hov" style="">
                        ...
                        <td class="text-center"> <a
                                onclick="showDetailModelOneParamerter('<?php        echo $detail?>','<?php        echo 'other' . ',' . $trow->voucher_no;?>','<?php        echo $PageTitle?>','<?php        echo $m?>','')"
                                class="btn btn-xs btn-success"><?php        echo date_format(date_create($trow->v_date), 'd-M-Y'); ?></a>
                        </td>
                        <td>{{$page_typ}}<br>{{$category}}</td>

                        <td class="text-left">
                            <?php        echo $cheque_no . '</br>';
                if ($cheque_date != ''):
                    ...
                    $all_debit += $trow->amount;
                } ?></td>
                        <td class="text-right">
                            <?php        if ($trow->debit_credit == 0) {
                    $credit = $trow->amount;
                    echo number_format($trow->amount, 2);
                    $total_credit += $trow->amount;
                    $all_credit += $trow->amount;
                } ?>
                        </td>
                        <?php
                ...
                if ($balance >= 0):
                    ...
                    <tr>
                        ...
                        <td class="text-right" colspan="1"><b
                                style="font-size: large;"><?php    echo number_format($total_debit, 2) ?></b></td>
                        <td class="text-right" colspan="1"><b
                                style="font-size: large;"><?php    echo number_format($total_credit, 2) ?></b></td>
