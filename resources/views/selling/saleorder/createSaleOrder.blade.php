    ...
    <div class="row well_N align-items-center">
        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
            <ul class="cus-ul">
                ...
                <li>
                    <h3><span class="glyphicon glyphicon-chevron-right"></span> &nbsp; Create Sales Order</h3>
                </li>
            </ul>
        </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="panel">
                        <div class="panel-body">
                            <form action="{{route('salesorder.store')}}" method="post" id="create-sale-order-form">
                                    ...
                                    <div class="tabel-responsive">
                                        <table class="userlittab table table-bordered sf-table-list" id="more_details">
                                            ...
                                            <tbody>
                                                <tr class="main">
                                                    <td>
                                                        <select style="width: 150px;" onChange="checkCustomerSelected(this);get_product_by_brand(this,1);getBrandCustomerDiscount(this,1);" name="brand_id[]" class="form-control brands" id="brand_id1" >
                                                    ...
                                                    <td>
                                                       <input style="width: 70px;" readonly name="c_stock[]" class="form-control c_stock" value="0" type="text" id="c_stock" tabindex="-1">
                                                    </td>
                                                    <td>
...
function get_product_by_brand(element, number) {
    const brandId = element.value;
    const $select = $("#product_id" + number);

    // Sabse pehle purana content saaf karo
    $select.empty();

    let htmlContent = '<option value="">First select brand</option>';

    if (brandId && productHash[brandId] && productHash[brandId].length > 0) {
        htmlContent = productHash[brandId].join("");   // ← yehi aapka purana HTML string array hai
    }

    // Ek baar mein daal do + requestAnimationFrame se bilkul smooth
    requestAnimationFrame(() => {
        $select.html(htmlContent);

        // Agar Select2 pehle se bana hai → sirf refresh karo (re-initialize mat karo!)
        if ($select.hasClass("select2-hidden-accessible")) {
            $select.val(null).trigger("change.select2");   // ← yeh line magic hai
        } else {
            // Pehli baar Select2 bana do
            $select.select2({
                ...
                language: {
                    noResults: function() {
                        return "Type keywords to search items";           // ← YEH APNA MESSAGE
                        // ya English mein: "Please select a brand first"
                    }
                }
            });
        }
