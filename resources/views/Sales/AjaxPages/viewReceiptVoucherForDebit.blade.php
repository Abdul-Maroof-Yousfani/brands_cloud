...
$m = $_GET['m'];
$id = $_GET['id'];
$Checking = $Checking = strpos($id, ',') !== false ? explode(',', $id)[1] : $id;

    FinanceHelper::companyDatabaseConnection($m);
    $rvs = DB::table('credits_data')->where('rv_no','=',$Checking)->first();
    FinanceHelper::reconnectMasterDatabase();
    $id = $rvs->id;
$currentDate = date('Y-m-d');
FinanceHelper::companyDatabaseConnection($m);
$rvs = DB::table('credits_data')->where('id','=',$id)->get();
FinanceHelper::reconnectMasterDatabase();
foreach ($rvs as $row) {
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="table-responsive">
                                <table  class="table table-bordered table-striped table-condensed tableMargin" style="display: none">
                                    <thead>
                                    <tr>
                                        <th class="text-center" style="width:50px;">S.No</th>
                                        <th class="text-center">Account</th>
                                    ...
                                    <?php
                                    FinanceHelper::companyDatabaseConnection($m);
                                    $rvsDetail = DB::table('credits_item_data')->where('rv_no',strtolower($Checking))->get();
                                    $costing_data=$rvsDetail;
                                    $type = 5;
                                    FinanceHelper::reconnectMasterDatabase();
                                    $counter = 1;
                                       ...
                                       ?>
                                        ...
                                        <?php $freight=SalesHelper::get_freight($row3->si_id); ?>
                                            <tr>
                                                ...
                                                @php
                                                    $invoice_amount = SalesHelper::get_total_amount_for_sales_tax_invoice_by_id($row3->si_id)->amount+$row3->sales_tax+$row3->sales_tax_further+$freight;
                                                @endphp
                                                <td class="text-right">{{number_format($invoice_amount,2)}}</td>
                                                <td class="text-right">{{ number_format($invoice_amount - $row3->net_amount, 2) }}</td>
                                            ...
                                            <?php
                                                ...
                                                $inv_amount = $invoice_amount = SalesHelper::get_total_amount_for_sales_tax_invoice_by_id($row3->si_id)->amount+$row3->sales_tax+$row3->sales_tax_further+$freight;
                                                $received_amount+=$inv_amount - $row3->net_amount;
