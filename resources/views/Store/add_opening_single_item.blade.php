...
@section('content')
    ...
    @if (Session::get('run_company') == 1 || Session::get('run_company') == 1)
        <?php echo Form::open(['url' => 'stad/insert_opening_data_single', 'id' => 'subm', 'class' => 'stop']); ?>
    @endif
    <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="col-lg-9 col-md-9  col-sm-9 col-xs-12" style="">
                                ...
                                <select class="form-control select2" name="sub_1" id="sub_1">
                                    ...
                                    @foreach (App\Helpers\CommonHelper::get_all_subitem_get() as $key => $value)
                                        <option value="{{ $value->id }}">
                                            {{ $value->sku_code  }} - {{ $value->product_name  }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <script>
            ...
            function get_data_opening() {
                ...
                $.ajax({
                    url: '{{ url('/pdc/get_data_opening_single') }}',
                    data: {
                        item: item
                    },
                    type: 'GET',
